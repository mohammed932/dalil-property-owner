...
export class UpdateActivityFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.activateRoute.paramMap
      .pipe(
        switchMap(x =>
          this.httpActivityService.getSingleActivity(
            x["params"]["id"],
            this.loadingData
          )
        )
  ...
  setFormData(activity) {
    ...
    this.configurationsForm.controls.terms_and_conditions_ar.setValue(
      activity.translation.ar.terms_and_conditions
    );
    this.configurationsForm.controls.terms_and_conditions.setValue(
  ...
  removeImg(index) {
    this.httpActivityService
      .deleteImageFromApi(this.activity._id, "image", this.urls[index])
      .subscribe(
        data => {
          if (data.status === 200) {
      ...
      .subscribe(
        data => {
          this.notificationService.successNotification(
            `تم تعديل النشاط ${this.configurationsForm.controls.name_ar.value}`
          );
          this.loading = false;
          this.isDisabled = false;
        },
